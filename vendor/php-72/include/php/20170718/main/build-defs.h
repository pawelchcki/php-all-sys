...
#define CONFIGURE_COMMAND " '../configure'  '--includedir=${prefix}/include' '--mandir=${prefix}/share/man' '--infodir=${prefix}/share/info' '--disable-silent-rules' '--libdir=${prefix}/lib/x86_64-linux-gnu' '--libexecdir=${prefix}/lib/x86_64-linux-gnu' '--disable-maintainer-mode' '--disable-dependency-tracking' '--prefix=/usr' '--enable-cli' '--disable-cgi' '--disable-phpdbg' '--with-config-file-path=/etc/php/7.2/cli' '--with-config-file-scan-dir=/etc/php/7.2/cli/conf.d' '--build=x86_64-linux-gnu' '--host=x86_64-linux-gnu' '--config-cache' '--cache-file=/build/php7.2-7.2.34/config.cache' '--libdir=\${prefix}/lib/php' '--libexecdir=\${prefix}/lib/php' '--datadir=\${prefix}/share/php/7.2' '--program-suffix=7.2' '--sysconfdir=/etc' '--localstatedir=/var' '--mandir=/usr/share/man' '--disable-all' '--disable-debug' '--disable-rpath' '--disable-static' '--with-pic' '--with-layout=GNU' '--without-pear' '--enable-filter' '--with-openssl=yes' '--with-password-argon2=/usr' '--with-pcre-regex=/usr' '--enable-hash' '--with-mhash=/usr' '--enable-libxml' '--enable-session' '--with-sodium' '--with-system-tzdata' '--with-zlib=/usr' '--with-zlib-dir=/usr' '--enable-dtrace' '--enable-pcntl' '--with-libedit=shared,/usr' 'build_alias=x86_64-linux-gnu' 'host_alias=x86_64-linux-gnu' 'CFLAGS=-g -O2 -fdebug-prefix-map=/build/php7.2-7.2.34=. -fstack-protector-strong -Wformat -Werror=format-security -O2 -Wall -pedantic -fsigned-char -fno-strict-aliasing -g'"
